...
export default function Resume() {
  ...
  return (
    <div className="space-y-8" data-testid="resume-page">
      <header>
        <div className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">Resume analyzer</div>
        <h1 className="mt-2 text-3xl md:text-4xl font-heading font-black tracking-tighter">ATS-grade resume review</h1>
        <p className="mt-2 text-muted-foreground">Upload PDF or DOCX. Gemini will score it, flag missing keywords, and rewrite weak bullets.</p>
      </header>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card className="p-8" data-testid="resume-upload-card">
          ...
          >
            ...
            <div className="mt-4 font-heading font-bold">Drop your resume here or click to browse</div>
          ...
          <Button onClick={onAnalyze} disabled={!file || busy} className="w-full mt-6 rounded-md font-semibold gap-2" data-testid="resume-analyze-btn">
            <Sparkles className="w-4 h-4" /> {busy ? "Analyzing with Gemini..." : "Analyze resume"}
          </Button>
        </Card>
        ...
        <Card className="p-8" data-testid="resume-results-card">
          ...
          {resume && a && (
            <div className="space-y-6">
              <div>
                <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Latest file</div>
                <div className="font-mono mt-1">{resume.filename}</div>
              </div>
              ...
              {a.summary && (
                <div className="border-l-2 border-primary pl-4 text-sm leading-relaxed">{a.summary}</div>
              )}
            </div>
          )}
        </Card>
      ...
      {resume && a && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6" data-testid="resume-feedback">
          {a.strengths?.length > 0 && (
            <Card className="p-6">
              <h3 className="font-heading font-bold text-lg flex items-center gap-2"><CheckCircle className="w-5 h-5 text-emerald-500" /> Strengths</h3>
              <ul className="mt-3 space-y-2 text-sm">{a.strengths.map((s, i) => <li key={i}>• {s}</li>)}</ul>
            </Card>
          )}
          {a.missing_skills?.length > 0 && (
            <Card className="p-6">
              <h3 className="font-heading font-bold text-lg flex items-center gap-2"><AlertCircle className="w-5 h-5 text-amber-500" /> Missing skills</h3>
          ...
          {a.missing_keywords?.length > 0 && (
            <Card className="p-6">
              <h3 className="font-heading font-bold text-lg">Missing keywords</h3>
          ...
          {a.improvement_suggestions?.length > 0 && (
            <Card className="p-6">
              <h3 className="font-heading font-bold text-lg">Improvement suggestions</h3>
              <ul className="mt-3 space-y-2 text-sm">{a.improvement_suggestions.map((s, i) => <li key={i}>• {s}</li>)}</ul>
            </Card>
          )}
          {a.weak_bullets?.length > 0 && (
            <Card className="p-6 md:col-span-2">
              <h3 className="font-heading font-bold text-lg">Bullet point rewrites</h3>
          ...
          {a.grammar_issues?.length > 0 && (
            <Card className="p-6">
              <h3 className="font-heading font-bold text-lg">Grammar issues</h3>
              <ul className="mt-3 space-y-2 text-sm">{a.grammar_issues.map((s, i) => <li key={i}>• {s}</li>)}</ul>
            </Card>
          )}
          {a.missing_metrics?.length > 0 && (
            <Card className="p-6">
              <h3 className="font-heading font-bold text-lg">Add metrics</h3>
